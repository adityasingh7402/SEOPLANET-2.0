    ...
    >
      ...
      <body className="min-h-full flex flex-col font-mono-pro">
        <Script 
          ...
          dangerouslySetInnerHTML={{ 
            __html: `window.IS_BOT = navigator.webdriver || (navigator.plugins && navigator.plugins.length === 0) || /bot|googlebot|crawler|spider|robot|crawling|lighthouse|chrome-lighthouse|ptst/i.test(navigator.userAgent); window.addEventListener("error", function (e) { if (e.error && e.error.name === "DataCloneError" && e.message && e.message.includes("PerformanceServerTiming")) { e.stopImmediatePropagation(); e.preventDefault() } }, true);` 
          }} 
        />
